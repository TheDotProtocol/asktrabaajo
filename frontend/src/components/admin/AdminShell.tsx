'use client';

import Link from 'next/link';
import { usePathname } from 'next/navigation';
import { ReactNode, useEffect, useState } from 'react';
import {
  Activity,
  Bell,
  Cpu,
  FileText,
  Gavel,
  Home,
  Landmark,
  LogOut,
  Menu,
  Scale,
  Settings,
  Shield,
  Sparkles,
  Users,
  Wallet,
  X,
} from 'lucide-react';

import { useCanonicalAuth } from '@/context/AuthContext';
import { api } from '@/lib/api/session';
import { hasPermission } from '@/lib/api/portal';
import { PortalSwitchLinks } from '@/components/os/PortalSwitchLinks';

type NavItem = {
  href: string;
  label: string;
  icon: typeof Home;
  permission?: string;
  foundation?: boolean;
};

const CONTROL: NavItem[] = [
  { href: '/admin', label: 'Command Center', icon: Home },
  { href: '/admin/governance', label: 'Governance', icon: Landmark, permission: 'reports.read' },
  { href: '/admin/governance/enforcement', label: 'Enforcement', icon: Gavel, permission: 'enforcement.read' },
  { href: '/admin/governance/appeals', label: 'Appeals', icon: Scale, permission: 'appeals.read' },
  { href: '/admin/governance/audit', label: 'Audit', icon: Activity, permission: 'platform.audit.read' },
  { href: '/admin/governance/teams', label: 'Teams', icon: Users, permission: 'reports.teams' },
];

const OPERATIONS: NavItem[] = [
  { href: '/admin/support', label: 'Support', icon: Shield },
  { href: '/admin/finance', label: 'Finance', icon: Wallet, permission: 'finance.read' },
  { href: '/admin/operations', label: 'Operations', icon: Cpu },
  { href: '/admin/athena', label: 'Athena', icon: Sparkles, foundation: true },
];

const FOOTER: NavItem[] = [
  { href: '/admin/notifications', label: 'Notifications', icon: Bell },
  { href: '/admin/settings', label: 'Settings', icon: Settings },
];

function isActive(pathname: string, href: string) {
  if (href === '/admin') return pathname === '/admin';
  return pathname === href || pathname.startsWith(`${href}/`);
}

function NavLink({
  item,
  pathname,
  onClick,
}: {
  item: NavItem;
  pathname: string;
  onClick?: () => void;
}) {
  const active = isActive(pathname, item.href);
  const Icon = item.icon;
  return (
    <Link
      href={item.href}
      onClick={onClick}
      className={`flex items-center gap-3 rounded-lg border px-3 py-2 text-[13px] transition-colors ${
        active
          ? 'border-[#23272a] bg-[#22242b] font-semibold text-white'
          : item.foundation
            ? 'border-transparent text-[#d4af37] hover:bg-white/[0.03]'
            : 'border-transparent text-[#9ca3af] hover:bg-white/[0.03] hover:text-white'
      }`}
    >
      <Icon className="size-4 shrink-0" strokeWidth={1.75} />
      <span className="flex-1">{item.label}</span>
      {item.foundation && (
        <span className="font-mono text-[9px] uppercase tracking-wider text-[#d4af37]/70">Soon</span>
      )}
    </Link>
  );
}

export function AdminShell({ children }: { children: ReactNode }) {
  const pathname = usePathname();
  const { me, logout } = useCanonicalAuth();
  const [open, setOpen] = useState(false);
  const [unread, setUnread] = useState(0);

  useEffect(() => {
    setOpen(false);
  }, [pathname]);

  useEffect(() => {
    let cancelled = false;
    api
      .get<{ unread: number }>('/jobseeker/notifications/unread-count')
      .then((row) => {
        if (!cancelled) setUnread(row.unread ?? 0);
      })
      .catch(() => {
        if (!cancelled) setUnread(0);
      });
    return () => {
      cancelled = true;
    };
  }, [pathname]);

  const visible = (items: NavItem[]) =>
    items.filter((item) => !item.permission || hasPermission(me, item.permission));

  const roleLabel =
    me?.memberships.find((m) => m.organization_kind === 'platform')?.role?.replaceAll('_', ' ') ??
    (me?.super_admin ? 'super admin' : 'platform operator');

  const sidebar = (
    <aside className="flex h-full w-[240px] shrink-0 flex-col justify-between border-r border-[#1e2026] bg-[#0d0e10] p-4">
      <div className="flex flex-col gap-6 overflow-y-auto">
        <div>
          <Link href="/admin" className="flex items-center gap-2">
            <span className="size-[18px] rounded-[3px] bg-[#d4af37] shadow-[0_0_8px_#d4af37]" />
            <span className="text-lg font-semibold tracking-tight text-white">
              Ask<span className="text-[#d4af37]">Trabaajo</span>
            </span>
          </Link>
          <div className="mt-3 flex items-center justify-between">
            <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#6b7280]">Global Admin</p>
            <span className="rounded border border-[#d4af37]/50 px-2 py-0.5 font-mono text-[10px] uppercase text-[#d4af37]">
              Development
            </span>
          </div>
        </div>
        <nav className="flex flex-col gap-1" aria-label="Control plane">
          <p className="mb-1 font-mono text-[10px] uppercase tracking-[0.18em] text-[#6b7280]">Control plane</p>
          {visible(CONTROL).map((item) => (
            <NavLink key={item.href} item={item} pathname={pathname} onClick={() => setOpen(false)} />
          ))}
        </nav>
        <nav className="flex flex-col gap-1" aria-label="System operations">
          <p className="mb-1 font-mono text-[10px] uppercase tracking-[0.18em] text-[#6b7280]">System operations</p>
          {visible(OPERATIONS).map((item) => (
            <NavLink key={item.href} item={item} pathname={pathname} onClick={() => setOpen(false)} />
          ))}
        </nav>
      </div>
      <div className="flex flex-col gap-3">
        <nav className="flex flex-col gap-1" aria-label="Account">
          {visible(FOOTER).map((item) => (
            <div key={item.href} className="relative">
              <NavLink item={item} pathname={pathname} onClick={() => setOpen(false)} />
              {item.href === '/admin/notifications' && unread > 0 && (
                <span className="absolute right-2 top-2 min-w-4 rounded-full bg-[#d4af37] px-1 text-center font-mono text-[10px] text-black">
                  {unread > 9 ? '9+' : unread}
                </span>
              )}
            </div>
          ))}
        </nav>
        <div className="rounded-lg border border-[#23272a] bg-[#111315] p-3">
          <p className="truncate text-sm font-medium text-white">{me?.full_name ?? 'Account'}</p>
          <p className="truncate font-mono text-[10px] uppercase tracking-wider text-[#9ca3af]">{roleLabel}</p>
          <div className="mt-2">
            <PortalSwitchLinks compact />
          </div>
          <div className="mt-3 flex items-center justify-between">
            <button
              type="button"
              onClick={() => void logout().then(() => { window.location.href = '/login'; })}
              className="inline-flex items-center gap-1.5 text-xs text-[#9ca3af] hover:text-white"
            >
              <LogOut className="size-3.5" /> Log out
            </button>
            <Link href="/id" className="text-xs text-[#9ca3af] hover:text-white" aria-label="Account settings">
              <FileText className="size-3.5" />
            </Link>
          </div>
        </div>
      </div>
    </aside>
  );

  return (
    <div className="candidate-os min-h-screen bg-[#0b0c0d] text-white">
      <div className="flex min-h-screen">
        <div className="hidden lg:block">{sidebar}</div>
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="sticky top-0 z-20 flex items-center justify-between border-b border-[#23272a] bg-[#0b0c0d]/90 px-4 py-3 backdrop-blur lg:hidden">
            <button
              type="button"
              className="rounded-md p-2 text-[#9ca3af]"
              aria-label={open ? 'Close menu' : 'Open menu'}
              onClick={() => setOpen((v) => !v)}
            >
              {open ? <X className="size-5" /> : <Menu className="size-5" />}
            </button>
            <span className="text-sm font-semibold">
              Ask<span className="text-[#d4af37]">Trabaajo</span>
            </span>
            <Link href="/admin/notifications" className="relative p-2 text-[#9ca3af]">
              <Bell className="size-5" />
              {unread > 0 && <span className="absolute right-1 top-1 size-2 rounded-full bg-[#d4af37]" />}
            </Link>
          </header>
          {open && (
            <div className="fixed inset-0 z-30 lg:hidden">
              <button className="absolute inset-0 bg-black/60" aria-label="Close" onClick={() => setOpen(false)} />
              <div className="relative h-full">{sidebar}</div>
            </div>
          )}
          <main className="min-w-0 flex-1 px-4 py-8 sm:px-8 lg:px-10">{children}</main>
        </div>
      </div>
    </div>
  );
}
